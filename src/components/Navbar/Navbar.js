...
const Navbar = () => {
    const globalQuantity = useSelector(state => state.globalQuantity);
    const showNavbar = () => {
        document.getElementsByClassName('nav-list-burger')[0].classList.remove('hide');
        document.getElementsByClassName('burger-icon')[0].classList.add('none');
        document.getElementsByClassName('burger-close')[0].classList.remove('none');
        document.getElementsByClassName('body-cart')[0].classList.remove('none');

    }
    const hideNavbar = () => {
        document.getElementsByClassName('nav-list-burger')[0].classList.add('hide');
        document.getElementsByClassName('burger-icon')[0].classList.remove('none');
        document.getElementsByClassName('burger-close')[0].classList.add('none');
    ...
    useEffect(() => {
        document.getElementsByClassName('body-cart')[0].addEventListener('click', () => {
            document.getElementsByClassName('body-cart')[0].classList.add('none')
        ...
        <main>
            <div className="container">
                <nav className="navbar">
                    <i className="burger-close none" onClick={() => hideNavbar()}><AiOutlineClose /></i>
                    <i className="burger-icon" onClick={() => showNavbar()}><GiHamburgerMenu /></i>
                    <Link to='/'>
                    ...
                    <ul className="nav-list">
                        ...
                        <li className="list"><Link to="/headphones">HEADPHONES</Link></li>
                        <li className="list"><Link to="/speakers">SPEAKERS</Link></li>
                        <li className="list"><Link to="/earphones">EARPHONES</Link></li>
                    </ul>
                    <div className="shop">
                        {globalQuantity !== 0 ? <span className="quantity-number">{globalQuantity}</span> : ''}
                        <i className="shop-icon" onClick={() => showCart()}><AiOutlineShoppingCart /></i>
                    </div>
                </nav>
                <article className="nav-list-burger hide">
                    <Link to="/headphones" className="list-burger" onClick={() => hideNavbar()}><div className="burger-box" >
                        <img src="../../../assets/shared/desktop/image-category-thumbnail-headphones.png" />
                        <h4>HEADPHONES</h4>
                        <span className="shop">shop</span>
                    </div>
                    </Link>
                    <Link to="/speakers" className="list-burger" onClick={() => hideNavbar()}><div className="burger-box">
                        <img src="../../../assets/shared/desktop/image-category-thumbnail-speakers.png" />
                        <h4>SPEAKERS</h4>
                        <span className="shop">shop</span>
                    </div>
                    </Link>
                    <Link to="/earphones" className="list-burger" onClick={() => hideNavbar()}><div className="burger-box">
